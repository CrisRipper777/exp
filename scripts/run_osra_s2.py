"""OSRA S2 driver: Ownership-Specific Relational Access variants Q0-Q4
(docs/OSRA_下一阶段推进计划.md, §14/§32).

Runs Q0-Q4 x Movies/Toys/Grocery x seeds 42/43/44 (=45 runs) under the
frozen unified protocol, STRICTLY VAL-ONLY (task.evaluate_test=false).
All variants share the S1 strongest Local parent (L3 factor-wise GATv2)
plus the single osra access code path + config switches.

Outputs go to outputs/osra/s2_access/<variant>/<dataset>/seed_<s>/ with
hydra results.json + history.csv + train.log + run_info.json. Resume/skip
on existing results.json (--force overrides); failures recorded.

Usage:
    python scripts/run_osra_s2.py --gpus 0,1
    python scripts/run_osra_s2.py --variants Q4 --datasets Movies --seeds 42
"""
...
# Shared parent = S1 strongest local parent (L3, factor-wise GATv2)
PARENT = ["model.osra.local_mode=gatv2"]
VARIANTS: dict[str, list[str]] = {
    "Q0": [*PARENT, "model.osra.cross_access=false"],
    "Q1": [*PARENT, "model.osra.cross_access=true", "model.osra.access_mode=direct"],
    "Q2": [*PARENT, "model.osra.cross_access=true", "model.osra.access_mode=query",
           "model.osra.query_mode=static", "model.osra.use_null=false"],
    "Q3": [*PARENT, "model.osra.cross_access=true", "model.osra.access_mode=query",
           "model.osra.query_mode=target", "model.osra.use_null=false"],
    "Q4": [*PARENT, "model.osra.cross_access=true", "model.osra.access_mode=query",
           "model.osra.query_mode=target", "model.osra.use_null=true"],
}

DATASETS = ["Movies", "Toys", "Grocery"]
SEEDS = [42, 43, 44]
OUT_ROOT = PROJECT_ROOT / "outputs" / "osra" / "s2_access"
...
def _run_job(variant: str, dataset: str, seed: int, gpu_id: int, force: bool,
             gpu_locks: dict[int, _WeightedSemaphore]) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="OSRA S2 driver (Q0-Q4 x M/T/G x 3 seeds, val-only)")
    ...
    jobs = sorted([(v, d, s) for v in variants for d in datasets for s in seeds])
    ...
    with ThreadPoolExecutor(max_workers=len(gpus) * slots) as executor:
        ...
        for variant, dataset, seed in jobs:
            futures[executor.submit(
                _run_job, variant, dataset, seed, next(gpu_iter), args.force, gpu_locks
