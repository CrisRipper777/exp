"""Run the strict R5-0b global-relay audit matrix.

This runner has its own output root and does not read or overwrite R5-0
artifacts.  Formal jobs are validation-only and are serialized per GPU to keep
full-graph memory predictable.
"""
...
VARIANTS = {
    ...
    "B2-FSEP4": [
        ...
        "model.scope.global_assignment_mode=learned",
    ...
    "B3-FSEP1": [
        ...
        "model.scope.reconciliation.mode=none",
        "model.scope.use_source_projection=false",
        "model.scope.global_num_slots=1",
        "model.scope.global_assignment_mode=learned",
        "model.scope.global_scale_init=0.05",
    ],
    "B4-UNIFORM4": [
        ...
        "model.scope.use_source_projection=false",
        "model.scope.global_num_slots=4",
        "model.scope.global_assignment_mode=uniform",
        "model.scope.global_scale_init=0.05",
    ],
}
...
def _write_master(relative_root: str, variants: list[str], datasets: list[str], seeds: list[int]) -> None:
    ...
    path = summary_dir / "R5_0B_MASTER_TABLE.csv"
    fields = [
        "variant", "dataset", "seed", "best_epoch", "best_val_acc_pct",
        "best_val_f1_pct", "params", "peak_gpu_mb", "runtime_sec", "implementation_commit",
    ]
...
def main() -> None:
    ...
    if args.smoke:
        _run_jobs(
            [("B2-FSEP4", "Movies", 42)], gpus, 1, 1,
            "smoke_movies_1ep", args.force,
        )
        _run_jobs(
            [("B2-FSEP4", "ele-fashion", 42)], gpus, 2, 2,
