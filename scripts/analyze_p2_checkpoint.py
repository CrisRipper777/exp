"""P2 best-checkpoint mechanism diagnostics (plan §45).

Loads a biaxis_p2 checkpoint, reruns the dataset, computes
``Model.compute_p2_diagnostics`` on the full graph. Outputs:
...
Usage:
    python scripts/analyze_p2_checkpoint.py --dataset Movies --seed 42 \
        --ckpt outputs/p2/screen/Movies/adaptive_uot/model.pt \
        --out outputs/p2/screen/Movies/adaptive_uot --device cuda:0 \
        --model-overrides model.p2.mode=adaptive_uot
"""
...
def _resolve_cfg(dataset: str, task: str, seed: int, model_overrides: list[str] | None):
    overrides = [f"dataset={dataset}", f"task={task}", f"seed={int(seed)}", "model=biaxis_p2"] + list(
        model_overrides or []
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="P2 checkpoint mechanism diagnostics")
    ...
    from src.models.biaxis_p2 import Model
    ...
    diag = model.compute_p2_diagnostics(x, edge_index)
    ...
    print(
        f"[diag] {args.dataset} seed={args.seed} K_eff={diag['relation']['effective_num']:.3f} "
        f"S_R={diag['relation']['specialization']:.3f} capacity_kl={diag['capacity_kl']:.4f}",
        flush=True,
    )
    for name, stats in diag["plan"].items():
        print(
    ...
    print(
        f"[diag] rel_conf={diag['relation_confidence']['mean']:.3f} "
        f"theta={diag['theta']['mean']:.3f}",
        flush=True,
    )
    if x.is_cuda:
