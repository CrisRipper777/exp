"""OSRA S1 driver: Strong Local Parent variants L0-L3
(docs/OSRA_下一阶段推进计划.md, §7/§30).

Runs L0-L3 x Movies/Toys/Grocery x seeds 42/43/44 (=36 runs) under the
frozen unified protocol, STRICTLY VAL-ONLY (task.evaluate_test=false).
All variants share the single osra code path + config switches.

Outputs go to outputs/osra/s1_local/<variant>/<dataset>/seed_<s>/ with
...
Usage:
    python scripts/run_osra_s1.py --gpus 0,1
    python scripts/run_osra_s1.py --variants L1 --datasets Movies --seeds 42
"""
...
VARIANTS: dict[str, list[str]] = {
    "L0": ["model.osra.local_mode=r3_diag"],
    "L1": ["model.osra.local_mode=strong", "model.osra.anchor_mix=0.0"],
    "L2": ["model.osra.local_mode=strong", "model.osra.anchor_mix=0.1"],
    "L3": ["model.osra.local_mode=gatv2"],
}

DATASETS = ["Movies", "Toys", "Grocery"]
SEEDS = [42, 43, 44]
OUT_ROOT = PROJECT_ROOT / "outputs" / "osra" / "s1_local"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="OSRA S1 driver (L0-L3 x M/T/G x 3 seeds, val-only)")
