"""Run the validation-only R5-1 OSCI matrix.

The four variants share the same model construction and optimizer protocol;
only the composer inputs differ.  Jobs are serialized per GPU to limit
full-graph memory.  ``evaluate_test=false`` is passed explicitly and the
runner never selects or reports a Test metric.
"""
...
def _run_one(
    ...
    cmd = [
        ...
        f"dataset={dataset}", "task=nc", "model=biaxis_osci",
        "num_runs=1", f"seed={seed}", "device=cuda:0",
        "task.evaluate_test=false", f"task.patience={patience}",
        f"task.epochs={epochs}", "task.eval_every=1",
        f"task.history_path={outdir / 'history.csv'}",
        f"task.save_ckpt_path={outdir / 'best.pt'}",
        f"hydra.run.dir={outdir / 'hydra'}",
        f"model.osci.mode={VARIANTS[variant]}",
        # R5-1 protocol is explicit here rather than inherited from a model
        # preset, so every C0--C3 run has identical structural controls.
        ...
        "model.osci.composer_hidden_dim=256",
        "model.osci.cross_scale_init=0.05",
    ]
    ...
    if process.returncode:
        ...
        with (failure_dir / "formal_failures.jsonl").open("a", encoding="utf-8") as handle:
            handle.write(json.dumps({**info, "log_tail": log_text[-4000:]}, ensure_ascii=False) + "\n")
        print(f"{tag} FAILED rc={process.returncode}; see {log_path}", flush=True)
    else:
        print(f"{tag} OK runtime={runtime:.0f}s peak={memory.get('peak')}MB", flush=True)
...
def _write_master(relative_root: str, variants: list[str], datasets: list[str], seeds: list[int]) -> Path:
    rows: list[dict] = []
    ...
    path = summary_dir / "R5_1_MASTER_TABLE.csv"
    fields = [
        "variant", "mode", "dataset", "seed", "best_epoch", "best_val_acc_pct",
        "best_val_f1_pct", "params", "peak_gpu_mb", "runtime_sec",
        "implementation_commit", "evaluate_test",
    ]
...
def main() -> None:
    ...
    if args.smoke:
        smoke_variants = list(VARIANTS)
        _run_jobs(
            [(variant, "Movies", 42) for variant in smoke_variants],
            gpus, 1, 1, "smoke_movies_1ep", args.force,
        )
        _run_jobs(
            [("C3-OSCI", "ele-fashion", 42)],
            gpus, 2, 2, "smoke_ele_2ep", args.force,
        )
        return
